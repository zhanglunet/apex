...
当前状态：正在开发，核心能力已合入主分支。`v0.1.1` 已补齐 Failure Card 生成 Eval Case 的幂等保护，避免重复沉淀相同回归样本。`v0.1.2` 增加 Eval Cases 状态筛选和状态更新，让回归样本池可以运营。`v0.1.3` 在 Dashboard 增加质量健康指标，让阻断质量项、Open Failure 和 Active Eval 可以一屏看到。
